...
(function() {
  var cyclicConnection, edge, getTemplate, identicalConnectionExists, modelViewConnection,
    slice = [].slice,
  ...
  App.Templates.constraintErrorModal = {
    ...
    bindEvents: function() {
      ...
      this.on("show.bs.modal", (function(_this) {
        return function() {
          content.html(_this.data("error"));
          return true;
        };
      })(this));
      return this;
  ...
  cyclicConnection = function(editor, umlClass, connection) {
    var c, k, len, ref, type;
    type = connection.type;
    if (type === "generalization" || type === "realization") {
      ref = editor.getClass(connection.target).outConnections;
      for (k = 0, len = ref.length; k < len; k++) {
        c = ref[k];
        type = c.type;
        if ((type === "generalization" || type === "realization") && c.target === umlClass.name) {
          return true;
        }
      }
    }
  ...
  identicalConnectionExists = function(editor, umlClass, connection) {
    var c, id, k, len, ref;
    id = connection.getId();
    ref = umlClass.outConnections;
    for (k = 0, len = ref.length; k < len; k++) {
      c = ref[k];
      if (c.getId() === id) {
        return true;
  ...
  modelViewConnection = function(editor, umlClass, connection) {
    var target;
    target = editor.getClass(connection.target);
    if ((umlClass.type === "model" && target.type === "view") || (umlClass.type === "view" && target.type === "model")) {
      return true;
    }
    return false;
  };

  edge = function(editor, umlClass, connection) {
    var base, name1;
    ...
    return (typeof (base = this.edge)[name1 = connection.type] === "function" ? base[name1](editor, umlClass, connection) : void 0) || true;
  };

  edge.realization = function(editor, umlClass, connection) {
  ...
  edge.generalization = function(editor, umlClass, connection) {
    if (cyclicConnection(editor, umlClass, connection)) {
      return "cycle detected";
    }
    return true;
  ...
  App.UMLClass = (function() {
    ...
    UMLClass.prototype.checkConnection = function(connection) {
      return App.UMLConstraints.edge(this.editor, this, connection);
    };

    UMLClass.prototype.addConnection = function(connection) {
      var checkResult;
      checkResult = this.checkConnection(connection);
      if (checkResult === true) {
        this.outConnections.push(connection);
      } else {
        this.editor.showError("UML constraint for <strong>" + (connection.type._capitalize()) + "</strong> not satisfied!\n<br /><br />\n<strong>Error:</strong> " + checkResult);
      }
      return this;
    };
